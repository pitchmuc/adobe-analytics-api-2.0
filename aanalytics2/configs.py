...
def createConfigFile(destination: str = 'config_analytics_template.json',auth_type: str = "oauthV2",verbose: bool = False, company_id: Optional[str] = None, rsid: Optional[str] = None, proxy: Optional[str] = None) -> None:
    ...
    Arguments:
        ...
        proxy : OPTIONAL : Proxy URL to pre-fill in the file (e.g. "http://proxy.example.com:8080"), for
                        organizations that need to reach the Adobe Analytics API through a proxy.
                        Applied to both http and https traffic. Not required — omit it (or leave it
                        out of the file) to connect without a proxy.
    """
    json_data = {
        'org_id': '<orgID>',
        'client_id': "<APIkey>",
...
def importConfigFile(path: str = None, auth_type: str = None, return_object: bool = False) -> Optional["ConfigObj"]:
    ...
    Arguments:
        ...
        auth_type : OPTIONAL : The type of Auth to be used by default. Detected from the config file if none is passed.
                        Possible value: "oauthV2"
    ...
    with open(config_file_path, 'r') as file:
        ...
        if 'proxy' in provided_keys:
            args["proxy"] = provided_config["proxy"]
...
def configure(org_id: str = None,
    ...
    Arguments:
        ...
        client_id : REQUIRED : The client_id (old api_key) provided by your Adobe Developer Console integration.
        ...
        proxy : OPTIONAL : Proxy to use for all requests (e.g. "http://proxy.example.com:8080"), for organizations
                        that need to reach the Adobe Analytics API through a proxy. Applied to both http
                        and https traffic. Not required — omit it to connect directly (no proxy).
        return_object : OPTIONAL : If True, returns an isolated config bundle dict with 'config' and 'header'
                        keys that can be passed directly to Login or Analytics via **bundle.
                        Does NOT update the global config state, enabling multiple credentials to coexist.
                        Default False (updates global state, returns None — backward-compatible behaviour).
